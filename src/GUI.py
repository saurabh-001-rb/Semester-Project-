...
# === Focus WhatsApp Window Once ===
st.write("📲 Focusing WhatsApp window...")
pyautogui.click(1252, 1053)  # Replace with WhatsApp window position
time.sleep(1)

# Neutral point to deselect text
...
def get_chat_text():
    ...
    time.sleep(0.5)
    pyautogui.click(NEUTRAL_CLICK_POINT)  # Deselect safely
    time.sleep(0.2)
...
# === Streamlit Main UI ===
st.title("🤖 WhatsApp Bot Controller")
st.write("This bot monitors incoming WhatsApp messages and automatically responds.")

# Button to start the bot
if st.button('Start Monitoring'):
    with st.spinner('Bot is starting...'):
        while True:
            try:
                    ...
                    if new_msgs:
                        ...
                        if not last_line.startswith(your_name + ":"):
                            ...
                            if reply:
                                ...
                                st.write(f"✅ Replied: {reply}")
                            else:
                                st.write("🛑 No valid reply generated.")
                        else:
                            st.write("🛑 Last message is from you. Skipping reply.")
